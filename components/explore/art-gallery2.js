    ...
    return (
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4'>
            {props.galleries.map((item, index) => (
                <div key={item.artId} className='w-full h-auto grid grid-cols-1 bg-[#1E1E1E] border-2 border-[#1E1E1E] dark:bg-white rounded-2xl text-white dark:text-gray-800 dark:border-2 dark:border-gray-200'>
                    <div className='grid grid-cols-1 gap-2 p-4'>
                        ...
                        <div className='flex flex-row'>
                            ...
                            <button className="rounded-full border-2 border-[#FFCC33] dark:border-0 dark:bg-[#FFCC33] w-[1.75rem] h-[1.75rem] text-white"  onClick={() =>
                    props.refresh
                      ? props.refresh(item.tokenId, item.itemId)
                      : ""
                    ...
                    <div className="border-t-2 border-[#FFCC33] dark:border-gray-200 p-4">
                       {item.sold?"":<button className='rounded-full bg-gradient-to-b from-[#ffffff] to-[#ffffff] text-black text-base w-full px-4 py-2 shadow-md' onClick={()=>props.children.buyFunction(item)} >⚡ {item.buttonTitle} </button>} 
                    </div>
                </div>
            ))}
        </div>
